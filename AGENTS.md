...
Stage-specific requirements, acceptance criteria, and scientific boundaries
belong in `docs/stages/`.

## Authority and current-stage work
...
A request to implement or complete the current QAQ stage authorizes one
complete current-stage delivery cycle:

1. inspect the current state and required prerequisites;
2. make reversible implementation choices inside the documented stage scope;
3. edit code, tests, configuration, and documentation required by the stage;
...
6. update durable project records when verified state or a material decision
   changed;
7. commit the completed stage work; and
...
Continue while the work remains inside the current stage, is reversible, and
...
- changing a scientific claim, frozen protocol, acceptance threshold,
  current-stage scope, non-goal, or preserved behavior;
- beginning a later stage;
...
- performing a real execution that the current stage or active task brief does
...
For GPU or other expensive execution, a stage request authorizes the real run
only when the current stage document or active task brief identifies the run,
