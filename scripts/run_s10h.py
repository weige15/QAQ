...
TRAIN_ROWS = (3, 1003, 2002, 3037, 4005, 5002, 6001, 7001, 8000, 9068, 10001, 11003, 12003, 13000, 14000, 15000, 16002, 17002, 18000, 19001, 20006, 21000, 22000, 23000)
...
def _validate_protocol(config: dict[str, Any]) -> None:
    ...
    _require(tuple(protocol["candidate_bits"]) == CANDIDATE_BITS, "REVISE", "candidate ordering drifted")
    _require(tuple(protocol["seeds"]) == SEEDS, "REVISE", "seed ordering drifted")
    _require(tuple(protocol["lambdas"]) == LAMBDAS and protocol["trial_count"] == 9, "REVISE", "trial matrix drifted")
    _require(protocol["required_ancestor_commit"] == REQUIRED_ANCESTOR, "REVISE", "required ancestor drifted")
    _require(config["prohibitions"]["production_lambda_selection"] is False, "REVISE", "production selection is not prohibited")
    _require(config["prohibitions"]["s10h_execution"] is False, "REVISE", "S10-H execution prohibition drifted")
    route_contract = config["protocol"]["future_measurements"]["route_map_contract"]
    _require(route_contract["validation_ids_in_order"] == list(VALIDATION_IDS), "REVISE", "validation route-map order drifted")
    _require(route_contract["units_per_map"] == 72 and route_contract["allowed_bits"] == list(CANDIDATE_BITS), "REVISE", "route-map contract drifted")
    _require(config["protocol"]["future_measurements"]["entropy_log_base"] == 2.0, "REVISE", "entropy base drifted")
...
def _validate_frozen_identity(config: dict[str, Any]) -> dict[str, Any]:
    ...
    _require(facts["pinned_model"] == {
        "repository": "Qwen/Qwen3-4B",
        "revision": MODEL_REVISION,
        "tokenizer_revision": MODEL_REVISION,
    }, "REVISE", "model or tokenizer identity drifted")
    _require(facts["pinned_backend"]["any_precision_revision"] == ANY_PRECISION_REVISION, "REVISE", "Any-Precision identity drifted")
    _require(facts["pinned_backend"]["packed_artifact_pytorch_model_sha256"] == ARTIFACT_SHA256, "REVISE", "packed artifact identity drifted")
    ...
    _require(manifest["source_model"]["repository"] == "Qwen/Qwen3-4B", "REVISE", "manifest model identity drifted")
    _require(manifest["source_model"]["revision"] == MODEL_REVISION, "REVISE", "manifest model revision drifted")
    _require(manifest["any_precision"]["commit"] == ANY_PRECISION_REVISION, "REVISE", "manifest backend revision drifted")
    ...
    revision_process = subprocess.run(["git", "-C", str(ANY_PRECISION_PATH), "rev-parse", "HEAD"], capture_output=True, text=True, check=False)
    if revision_process.returncode != 0 or revision_process.stdout.strip() != ANY_PRECISION_REVISION:
        raise ProtocolError("PAUSE", "pinned Any-Precision checkout is unavailable")
    dirty = subprocess.run(["git", "-C", str(ANY_PRECISION_PATH), "status", "--porcelain=v1"], capture_output=True, text=True, check=False)
...
def _validate_route_map(value: Any) -> bool:
    return (
        ...
        and len(value) == 72
        and all(isinstance(bit, int) and not isinstance(bit, bool) and bit in CANDIDATE_BITS for bit in value)
    )
...
def _aggregate_trials(trials: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "paired_control_hard_kd_delta_median_lambda_0.03_minus_lambda_0.0": statistics.median(kd_deltas),
        "paired_control_hard_width_delta_median_lambda_0.03_minus_lambda_0.0": statistics.median(width_deltas),
...
def _same_number(left: Any, right: Any) -> bool:
    return _finite_number(left) and _finite_number(right) and math.isclose(float(left), float(right), rel_tol=1e-12, abs_tol=1e-12)
...
def _validate_trial(trial: dict[str, Any]) -> tuple[bool, bool, str | None]:
    ...
    if not isinstance(prohibited, dict) or prohibited.get("passed") is not True or prohibited.get("forbidden_measurements_observed") != []:
    ...
    if trial["distinct_hard_route_map_count"] != unique or not _same_number(trial["hard_validation_mean_selected_bit_width"], width):
    ...
    numeric_fields = [key for key in REQUIRED_PER_TRIAL_FIELDS if key.endswith(("kd", "error", "width", "p4", "p6", "p8", "entropy", "ratio", "norm"))]
...
def validate_result(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(result, dict) or result.get("format") != "qaq-s10h-broader-validation-v1" or result.get("stage") != "S10-H":
    ...
    if not isinstance(protocol_identity, dict) or protocol_identity.get("config_sha256") != LOCKED_CONFIG_SHA256 or protocol_identity.get("config_byte_exact") is not True:
    ...
    if not isinstance(ancestry, dict) or ancestry.get("required_ancestor") != REQUIRED_ANCESTOR or ancestry.get("ancestor_ok") is not True:
    ...
    if not isinstance(identities, dict) or any(identities.get(key) != value for key, value in expected_identity.items()):
    ...
    if not isinstance(dataset, dict):
        ...
        for name, ids, rows, offsets in (
            ...
            if len(manifest) != len(ids):
                ...
                continue
            actual_ids = [item.get("example_id") if isinstance(item, dict) else None for item in manifest]
            actual_rows = [item.get("source_row") if isinstance(item, dict) else None for item in manifest]
            actual_offsets = [item.get("source_offset") if isinstance(item, dict) else None for item in manifest]
            if actual_ids != list(ids) or actual_rows != list(rows) or actual_offsets != list(offsets):
    ...
    if not isinstance(run_audits, dict):
        ...
        if not isinstance(inherited, dict) or not {"status", "test_selection", "passed"}.issubset(inherited):
            pause_errors.append("inherited regression audit is incomplete")
        elif inherited != {"status": "passed", "test_selection": "S10-D/S10-E/S10-F predecessor regression selection", "passed": True}:
            revise_errors.append("inherited regression audit failed or drifted")
        if not isinstance(prohibited, dict) or not {"forbidden_actions_observed", "forbidden_measurements_observed", "passed"}.issubset(prohibited):
            pause_errors.append("prohibited-work audit is incomplete")
        elif prohibited["passed"] is not True or prohibited["forbidden_actions_observed"] != [] or prohibited["forbidden_measurements_observed"] != []:
    ...
    if pause_errors:
        ...
        checks = {
            ...
            "no_invalid_or_degenerate_collapse": all(trial["collapse_audit"]["passed"] for trial in trials),
            ...
            "paired_kd": aggregates["paired_control_hard_kd_delta_median_lambda_0.03_minus_lambda_0.0"] <= 0.0,
            "paired_width": aggregates["paired_control_hard_width_delta_median_lambda_0.03_minus_lambda_0.0"] < 0.0,
        }
        # Explicitly apply REVISE before REFINE, then the two-axis thresholds.
        if not all(checks[key] for key in ("no_invalid_or_degenerate_collapse", "reproducibility_failures_zero")):
            classification = "REVISE"
...
def _plan(context: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "ancestry": {"required_ancestor": REQUIRED_ANCESTOR, "current_head": context["head"], "ancestor_ok": True},
        ...
        "trials": {"seeds": list(SEEDS), "lambdas": list(LAMBDAS), "ordered_pairs": [list(pair) for pair in TRIAL_PAIRS], "count": 9},
        "training_contract": protocol["training"],
        "future_output": {"format": "qaq-s10h-broader-validation-v1", "path": str(RESULT_PATH.relative_to(ROOT)), "exists": RESULT_PATH.exists(), "overwrite": "refuse"},
    ...
    for seed in SEEDS:
        for lambda_bit in LAMBDAS:
            ...
            trials.append({
                ...
                "collapse_audit": {"classification": "PROMPT_INVARIANT", "invalid_or_degenerate": False, "passed": True},
                ...
                "route_variation": {"prompt_count": 12, "unit_count": 72, "changed_unit_count": 0, "changed_fraction": 0.0},
                "distinct_hard_route_map_count": unique,
                "reproducibility_audit": {"route_maps_identical": True, "hard_metrics_identical": True, "finite_outputs_both_passed": True, "passed": True, "repeat_count": 1},
                "prohibited_measurement_audit": {"forbidden_measurements_observed": [], "passed": True},
            })
            serial += 1
    result = {
...
def main(argv: list[str] | None = None) -> int:
    ...
    mode.add_argument("--plan", action="store_true", help="validate and print the non-executing plan")
    mode.add_argument("--execute", action="store_true", help="explicit H2 opt-in; unavailable in S10-H1")
    parser.add_argument("--config", type=Path, default=CONFIG_PATH)
    parser.add_argument("--output", type=Path, default=RESULT_PATH)
    args = parser.parse_args(argv)
    try:
        context = _validate_pre_execution(config_path=args.config, result_path=args.output if args.execute else Path("/dev/null/noncanonical-result"))
        if args.execute:
            raise ProtocolError("PAUSE", "S10-H2 executor is intentionally unavailable; H1 performed validation only")
