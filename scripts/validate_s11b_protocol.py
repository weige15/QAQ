...
DEFAULT_CONFIG = ROOT / "configs" / "s11b_quality_pilot.json"
...
def _route_digest(request_id: str, route_logs: list[dict[str, Any]]) -> str:
    ...
    if set(by_key) != expected_keys:
        raise ProtocolValidationError(f"historical route coverage is not 72 unique units: {request_id}")
    records = [
        {
...
def _validate_modes(config: dict[str, Any]) -> None:
    ...
    if not isinstance(modes, list) or len(modes) != 2 or not all(
...
def _validate_authoritative_sources(config: dict[str, Any], root: Path) -> None:
    s09 = _read_json(root / "configs" / "s09_baseline_eval.json")
    s07_config = _read_json(root / "configs" / "s07_router_training.json")
    manifest = _read_json(root / "docs" / "quantized_model_manifest.json")
    s07_result = _read_json(root / "docs" / "results" / "s07_router_training.json")
    prompts = _read_json(root / "configs" / "s09_baseline_prompts.json")

    s09_model = _get(s09, "identities.model")
    manifest_model = _get(manifest, "source_model")
    result_model = _get(s07_result, "source_model")
    model = _get(config, "identities.model")
    tokenizer = _get(config, "identities.tokenizer")
    if model != {
        "repository": s09_model.get("repository"),
        "revision": s09_model.get("revision"),
    }:
    ...
    if tokenizer != {
        "repository": s09_model.get("tokenizer_repository"),
        "revision": s09_model.get("tokenizer_revision"),
    } or tokenizer != {
    ...
    if prompts.get("tokenizer", {}).get("repository") != tokenizer["repository"] or prompts.get(
        "tokenizer", {}
    ).get("revision") != tokenizer["revision"]:
        raise ProtocolValidationError("fixed-input tokenizer identity drifted")

    packed = _get(config, "identities.packed_artifact")
    s09_packed = _get(s09, "identities.packed_artifact")
    expected_packed = {
        "manifest_path": s09_packed.get("manifest_path"),
        "relative_path": s09_packed.get("relative_path"),
        "checkpoint_file": s09_packed.get("checkpoint_file"),
        "sha256": s09_packed.get("sha256"),
    }
    ...
    s09_any = _get(s09, "identities.any_precision")
    if any_precision != {
        "submodule_path": s09_any.get("submodule_path"),
        "commit": s09_any.get("manifest_commit"),
    }:
    ...
    s09_router = _get(s09, "identities.router")
    result_checkpoint = _get(s07_result, "checkpoint")
    if router != {
        "path_env_override": s09_router.get("checkpoint_path_env_override"),
        "recorded_external_path": result_checkpoint.get("external_path"),
        "sha256": s09_router.get("sha256"),
        "historical_result_path": "docs/results/s07_router_training.json",
        "historical_result_format": s07_result.get("format"),
    ...
    if (
        ...
        or s07_result.get("checkpoint_roundtrip", {}).get("expected_checkpoint_sha256")
        != router["sha256"]
        or s07_result.get("checkpoint_roundtrip", {}).get("hard_routes_match_recorded_result")
        is not True
    ):
        raise ProtocolValidationError("S07 checkpoint or historical hard-route evidence drifted")
    if _get(config, "identities.fixed_inputs_path") != _get(s09, "fixed_inputs.path"):
    ...
    if _get(config, "quality_metrics.teacher_student_kl.temperature") != _get(
        s07_config, "training.distillation_temperature"
    ):
    ...
    route_logs = s07_result.get("evaluation", {}).get("hard", {}).get("route_logs")
...
def _validate_exact_contract(config: dict[str, Any]) -> None:
    ...
    classifications = _get(
        config, "result_schema_contracts.aggregation.classification_values"
    )
