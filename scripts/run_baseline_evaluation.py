...
def main(argv: list[str] | None = None) -> int:
    ...
    action.add_argument(
        "--execute", action="store_true", help="launch one fresh child for each frozen mode"
    )
    action.add_argument("--execute-mode", metavar="MODE", help=argparse.SUPPRESS)
    action.add_argument(
        "--aggregate", action="store_true", help="validate five existing per-mode results"
    )
    ...
    try:
        if args.execute_mode:
            ...
            result = execute_mode(
                args.config.resolve(), args.execute_mode, args.output.resolve(), args.device
            )
            print(
                json.dumps(
                    {"mode_id": result["mode_id"], "output": str(args.output.resolve())},
                    sort_keys=True,
                )
            )
            return 0
        if args.aggregate:
            ...
            return (
                ...
                if result["classification"] == "PAUSE"
                else 1
            )
        if args.execute:
