...
PROMPTS = ROOT / "configs/static_quality_prompts.txt"
GENERATION_PROMPTS = ROOT / "configs/static_generation_prompts.txt"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output", type=Path, default=ROOT / "docs/results/s03_static_quality.json"
    )
    ...
    for precision in (4, 8):
        ...
        static_perplexity[str(precision)] = evaluate_perplexity(
            static_model, fp_windows, args.device
        )
    ...
    result = {
        ...
        "source_model": {
            ...
            "quantized_checkpoint_sha256": manifest["artifact"]["checkpoint_hashes"][
                "pytorch_model.bin"
            ],
        },
        "target_invariant": {
        ...
        "commands": {
            "quality": (
                "source ~/.venv/bin/activate && which python && python --version && "
                "python scripts/evaluate_static_model_quality.py"
            ),
