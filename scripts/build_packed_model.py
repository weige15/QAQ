...
def _checkpoint_inventory(artifact: Path, target_names: list[str], torch: Any) -> dict[str, Any]:
    ...
    for name in target_names:
        ...
        if list(lut4.shape) != [qweight.shape[1], 16] or list(lut8.shape) != [
            qweight.shape[1],
            256,
        ]:
    ...
    excluded_qweights = [
        key for key in qkeys if any(fragment in key for fragment in excluded_fragments)
    ]
    if excluded_qweights:
        raise ValueError(f"excluded modules were packed: {excluded_qweights}")
    original_target_weights = [
        f"{name}.weight" for name in target_names if f"{name}.weight" in state
    ]
    if original_target_weights:
        raise ValueError(
            f"original target weights remain in packed checkpoint: {original_target_weights[:3]}"
        )
...
def main() -> int:
    ...
    if (
        before["disk_free_bytes"] < expected_temporary
        or before["ram_available_bytes"] < expected_temporary
    ):
    ...
    if artifact_root.exists():
        if not args.overwrite_artifact:
            raise SystemExit(
                f"REVISE: artifact already exists; use --overwrite-artifact: {artifact_root}"
            )
    ...
    manifest = {
        ...
        "quantization": {
            ...
            "exact_command": (
                ...
                "python scripts/build_packed_model.py --overwrite-artifact"
            ),
        },
        "target_invariant": {
