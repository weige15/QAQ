...
def test_plan_validates_protocol_and_has_no_execution_side_effects(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "scripts.validate_baseline_evaluation_protocol.validate_protocol", fake_validate
    )
    details = runner.plan(ROOT / "configs/baseline_evaluation.json", tmp_path / "results", "cuda:3")
    ...
    assert (
        details["aggregation_command"][details["aggregation_command"].index("--aggregate")]
        == "--aggregate"
    )
    assert calls == [(ROOT / "configs/baseline_evaluation.json", True, True)]
