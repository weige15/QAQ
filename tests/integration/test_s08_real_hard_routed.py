...
def real_models():
    examples = run_s08b._load_examples()
    return examples, run_s08b._load_student("resident"), run_s08b._load_student("on_demand")


def test_real_on_demand_has_cpu_authority_and_no_hidden_packed_copy(real_models):
    examples, _, on_demand = real_models
    state, context = run_s08b._context_for(on_demand, examples[0], "s08-real-audit")
    ...
    assert not any(module.__class__.__name__ == "AnyPrecisionLinear" for module in on_demand.base.modules())
...
def test_real_hard_routes_and_transferred_execution_match(real_models):
    ...
    example = examples[0]
    input_ids = example.input_ids.unsqueeze(0).to(run_s08b.DEVICE)
    resident_output, resident_state, _, _ = run_s08b._hard_forward(
        resident,
        example,
    ...
    on_output, on_state, context, _ = run_s08b._hard_forward(
        on_demand,
        example,
    ...
    assert context is not None
    assert run_s08b._route_map(resident_state) == run_s08b._route_map(on_state)
    ...
    transfer = run_s08b._transfer_summary(
        context, run_s08b._route_map(on_state), prefill_count=len(context.records)
    )
...
def test_real_decode_reuses_and_later_request_transfers_again(real_models):
    examples, _, on_demand = real_models
    generation = run_s08b._generation(on_demand, examples[0], request_id="s08-real-generation")
    assert generation["all_finite"]
    assert generation["route_map_unchanged_during_decode"]
    assert generation["decode_transfer_bytes"] == 0
    _, state, context, _ = run_s08b._hard_forward(
        on_demand,
        examples[1],
        request_id="s08-real-later",
        input_ids=examples[1].input_ids.unsqueeze(0).to(run_s08b.DEVICE),
        use_cache=False,
    )
    assert context is not None
