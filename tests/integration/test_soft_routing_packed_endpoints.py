...
def test_real_qwen3_soft_endpoints_match_manual_and_static_execution(manual_case, static_case):
    manual_model, inputs, torch_module = manual_case
    static_model, static_inputs, _ = static_case
    for bits in (4, 8):
        state = QaqRequestState(
            f"s06-endpoint-{bits}", prompt_length=int(inputs["attention_mask"].sum())
        )
        ...
        assert torch_module.allclose(
            soft_logits.float(), static_logits.float(), atol=1e-3, rtol=1e-3
        )
