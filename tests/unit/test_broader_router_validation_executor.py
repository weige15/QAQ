...
from scripts import run_broader_router_validation as protocol
from scripts import train_baseline_router
...
class _TinyRuntime:
    ...
    def identity_evidence(self):
        return {
            ...
            "historical_s07_checkpoint_sha256": protocol.HISTORICAL_ROUTER_CHECKPOINT_SHA256,
        }
...
def _selected_examples(split):
    return train_baseline_router._select_examples(
...
def test_qwen_prepare_retains_real_selection_order_and_distillation_objects(monkeypatch):
    ...
    from scripts import calibrate_router_cost
    ...
    monkeypatch.setattr(train_baseline_router, "_device_example", device_example)
    monkeypatch.setattr(train_baseline_router, "_precompute_teacher_logits", precompute)
    monkeypatch.setattr(quality, "load_full_precision_model", load_teacher)
    monkeypatch.setattr(
        calibrate_router_cost,
