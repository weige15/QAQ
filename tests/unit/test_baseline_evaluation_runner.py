...
def _protocol():
    config, prompts, config_hash = runner.load_protocol(ROOT / "configs/baseline_evaluation.json")
...
def _fixture(
    mode_id: str, config: dict, prompts: dict, config_hash: str, *, ppl: float = 10.0
) -> dict:
    ...
    identities = {
        ...
        "any_precision_revision": runner.ANY_PRECISION_REVISION
        if mode["packed_artifact"]
        else None,
        "packed_checkpoint_sha256": config["identities"]["packed_artifact"]["sha256"]
        if mode["packed_artifact"]
        else None,
        "router_checkpoint_sha256": config["identities"]["router"]["sha256"]
    ...
    result = {
            ...
            "records": [
                {
                    ...
                    "output_digest": "fixture-output",
                    "logits_digest": "fixture-logits",
        ...
        "memory": {
            ...
            "physically_resident_packed_weight_bytes": 123
            if mode["packed_artifact"] and mode_id != runner.ON_DEMAND_MODE_ID
            else 0,
        ...
        "latency": {
            ...
            "median_seconds": {
                item["id"]: {"prefill": 1.0, "decode": 1.0, "end_to_end": 2.0} for item in requests
            },
        },
            ...
            "repeat_evidence": [
                {
                    ...
                    "route_map_digests": [_route(item["id"])["route_map_digest"]] * 5
                    if mode_id in runner.ROUTED_MODE_IDS
                    else [],
    ...
    if mode_id in runner.ROUTED_MODE_IDS:
        result["routed"] = {
            "requests": [_route(item["id"]) for item in requests],
            "route_diversity": {"adaptivity_classification": "OTHER"},
        }
    if mode_id == runner.ON_DEMAND_MODE_ID:
        result["on_demand"] = {
            ...
            "attention_bytes": 5,
            "ffn_bytes": 5,
            "total_transfer_bytes": 10,
            "first_use_events": 1,
            "reuse_events": 0,
            "independently_expected_physical_bytes": 10,
...
def test_environment_guard_and_frozen_config_are_explicit(monkeypatch):
    ...
    assert runner.frozen_generation_arguments(config) == {
        "batch_size": 1,
        "do_sample": False,
        "num_beams": 1,
        "max_new_tokens": 8,
...
def test_child_command_is_one_explicit_fresh_process_per_mode():
    config, _, _ = _protocol()
    commands = [
        runner.child_command(
            ROOT / "configs/baseline_evaluation.json",
            mode["id"],
            Path("tmp") / f"{mode['id']}.json",
            "cuda:3",
        )
        for mode in runner.resolve_modes(config)
    ]
    assert len(commands) == 5
    assert all("--execute-mode" in command for command in commands)
    assert {command[command.index("--execute-mode") + 1] for command in commands} == set(
        runner.EXPECTED_MODE_IDS
    )
...
def test_aggregation_quality_math_and_missing_results(tmp_path):
    config, prompts, config_hash = _protocol()
    paused = runner.aggregate(ROOT / "configs/baseline_evaluation.json", tmp_path)
    ...
    for mode in config["modes"]:
        result = _fixture(
            ...
            prompts,
            config_hash,
            ppl=10.0 if mode["id"] != runner.EXPECTED_MODE_IDS[2] else 11.0,
        )
        (tmp_path / f"{mode['id']}.json").write_text(json.dumps(result))
    aggregate = runner.aggregate(ROOT / "configs/baseline_evaluation.json", tmp_path)
    ...
    assert (
        runner.aggregate(ROOT / "configs/baseline_evaluation.json", tmp_path)["classification"]
        == "REVISE"
    )
...
def test_aggregation_rejects_unmeasured_cleanup_hidden_latency_hardware_perplexity_and_repeats(
    tmp_path,
):
    config, prompts, config_hash = _protocol()
    mutations = {
        "cleanup": lambda result: result["on_demand"].pop("cleanup_records"),
        "cleanup-hardcoded": lambda result: result["on_demand"].update(
            {
                "retained_entries_after_cleanup": 0,
                "cleanup_records": [
                    {
                        **result["on_demand"]["cleanup_records"][0],
                        "retained_entries_after_cleanup": 1,
                    }
                ],
            }
        ),
        "hidden-copy": lambda result: result["on_demand"].update(
            {"hidden_copy_audit": {"any_precision_module_count": 0}}
        ),
        "latency": lambda result: result["latency"]["median_seconds"]["s03-quality-0"].update(
            {"prefill": 99.0}
        ),
        ...
        "packed-identity": lambda result: result["identities"].update(
            {"any_precision_revision": "wrong"}
        ),
    ...
    for name, mutate in mutations.items():
        ...
        for mode in config["modes"]:
            ...
            if (
                ...
                or (mode["id"] == runner.EXPECTED_MODE_IDS[1] and name == "packed-identity")
            ):
                mutate(result)
            (case_dir / f"{mode['id']}.json").write_text(json.dumps(result))
        assert (
            runner.aggregate(ROOT / "configs/baseline_evaluation.json", case_dir)["classification"]
            == "REVISE"
        )
...
def test_on_demand_execute_mode_serializes_measured_cleanup_without_keyerror(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        runner,
        "_environment",
        lambda torch, transformers, device: _fixture(mode_id, config, prompts, config_hash)[
            "hardware"
        ],
    )
    monkeypatch.setattr(
        runner, "provenance", lambda: {"git_commit": "fixture", "worktree_status": ""}
    )
    monkeypatch.setattr(runner, "_seed", lambda torch, seed: None)
    monkeypatch.setattr(
        runner,
        "_expected_physical_bytes",
        lambda context, route_map: {"total": 0, "attention": 0, "ffn": 0},
    )
    ...
    monkeypatch.setattr(
        "qaq.model.static.file_sha256",
        lambda path: (
            config["identities"]["router"]["sha256"]
            if str(path) == str(checkpoint)
            else config["identities"]["packed_artifact"]["sha256"]
        ),
    )
    monkeypatch.setattr(
        "qaq.evaluation.quality.build_perplexity_windows",
        lambda tokenizer, sample_count, stride: (
            [],
            {
                **setup,
                "sample_count": 32,
    ...
    monkeypatch.setattr(
        "qaq.evaluation.quality.evaluate_perplexity",
        lambda model, windows, device: {
            "mean_negative_log_likelihood": 1.0,
            "perplexity": 2.0,
            "evaluated_token_count": 4096,
        },
    ...
    result = runner.execute_mode(
        ROOT / "configs/baseline_evaluation.json", mode_id, output_path, "cuda:3"
    )
