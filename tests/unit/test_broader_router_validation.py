...
from scripts import provision_packed_model_artifact
from scripts import run_broader_router_validation as runner
...
def test_artifact_provisioning_rejects_unverified_source(tmp_path):
    ...
    with pytest.raises(provision_packed_model_artifact.ArtifactProvisionError, match="SHA-256"):
        provision_packed_model_artifact.provision_artifact(
            source, tmp_path / "destination-artifact"
        )
...
def test_historical_frontier_result_mutation_is_rejected_without_rewrite(tmp_path, monkeypatch):
...
def test_cli_plan_is_deterministic_nonexecuting_and_subprocess_safe(tmp_path):
    ...
    command = [
        sys.executable,
        str(ROOT / "scripts/run_broader_router_validation.py"),
        "--plan",
        "--output",
        str(output),
    ]
