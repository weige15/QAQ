...
from scripts import verify_router_checkpoint_roundtrip
from scripts.verify_router_checkpoint_roundtrip import assert_actual_hard_routes_match
...
def test_checkpoint_identity_requires_recorded_hash_match(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        verify_router_checkpoint_roundtrip, "EXPECTED_CHECKPOINT_SHA256", "expected-hash"
    )
    monkeypatch.setattr(verify_router_checkpoint_roundtrip, "_sha256", lambda path: "expected-hash")

    with pytest.raises(SystemExit, match="checkpoint identity mismatch"):
        verify_router_checkpoint_roundtrip._verify_checkpoint_identity(result, result_path)
...
def test_roundtrip_failure_persists_failed_gate_fields(tmp_path):
    ...
    verify_router_checkpoint_roundtrip._persist_roundtrip_failure(
        result,
        result_path,
