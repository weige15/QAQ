...
from scripts import validate_s11b_protocol as validator

ROOT = Path(__file__).resolve().parents[2]
CONFIG = ROOT / "configs" / "s11b_quality_pilot.json"
...
def test_cli_is_deterministic_across_repeated_read_only_runs():
    command = [
        sys.executable,
        str(ROOT / "scripts" / "validate_s11b_protocol.py"),
        "--config",
        str(CONFIG),
    ]
...
def test_validator_writes_nothing_and_all_planned_results_remain_absent():
    watched = [
        CONFIG,
        ROOT / "configs" / "s09_baseline_eval.json",
        ROOT / "configs" / "s09_baseline_prompts.json",
...
def test_authoritative_identities_and_full_input_ids_agree():
    payload = _payload()
    s09 = json.loads((ROOT / "configs" / "s09_baseline_eval.json").read_text())
    manifest = json.loads((ROOT / "docs" / "quantized_model_manifest.json").read_text())
    s07 = json.loads((ROOT / "docs" / "results" / "s07_router_training.json").read_text())
    prompts = json.loads((ROOT / "configs" / "s09_baseline_prompts.json").read_text())
    identities = payload["identities"]

    assert identities["model"] == {
        "repository": s09["identities"]["model"]["repository"],
        "revision": manifest["source_model"]["revision"],
    }
    assert identities["tokenizer"]["revision"] == manifest["source_model"][
        "tokenizer_revision"
    ]
    assert identities["packed_artifact"]["relative_path"] == manifest["artifact"][
        "local_path"
    ]
    assert identities["packed_artifact"]["sha256"] == manifest["artifact"][
        "checkpoint_hashes"
    ]["pytorch_model.bin"]
    assert identities["router_checkpoint"]["sha256"] == s07["checkpoint"]["sha256"]
    ...
    for frozen in payload["fixed_inputs"]["requests"]:
        ...
        assert validator._token_digest(request["full_input_ids"]) == frozen[
            "token_digest_sha256"
        ]
...
def test_interpretation_paths_and_result_schemas_are_explicit_and_unique():
    ...
    assert payload["route_comparison"]["cross_mode_equality"][
        "ffn_equality_beyond_layer_0_required"
    ] is False
    ...
    assert all(not PurePosixPath(path).is_absolute() and ".." not in PurePosixPath(path).parts for path in paths)
    assert payload["result_schema_contracts"]["per_mode"]["schema"] == payload[
        "planned_results"
    ]["per_mode_schema"]
    assert payload["result_schema_contracts"]["aggregation"]["schema"] == payload[
        "planned_results"
    ]["aggregation_schema"]


def test_validator_imports_without_ml_or_backend_packages():
    script = ROOT / "scripts" / "validate_s11b_protocol.py"
    code = f"""
...
spec = importlib.util.spec_from_file_location('s11b_validator_isolated', {str(script)!r})
