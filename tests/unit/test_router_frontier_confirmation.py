...
from scripts import confirm_router_frontier as runner
from scripts.calibrate_router_cost import fresh_router_optimizer
...
def test_calibration_evidence_is_pinned_and_complete():
    config, result = runner._validate_calibration_evidence()
...
def test_semantic_internal_names_preserve_frozen_result_keys():
    calibration, calibration_result = runner._validate_calibration_evidence()
    result = runner._make_result(
        ...
        inherited_regressions_status="passed",
    )
    project_facts = result["project_established_facts"]
    assert project_facts["canonical_s10d_config_sha256"] == runner.LOCKED_CALIBRATION_CONFIG_SHA256
    assert project_facts["canonical_s10d_result_sha256"] == runner.LOCKED_CALIBRATION_RESULT_SHA256
    assert project_facts["canonical_s10d_result_commit"] == calibration_result["commit"]
    assert project_facts["inherited_s10d_config_format"] == calibration["format"]
    assert not any("calibration" in key for key in project_facts)


def test_execution_config_reuses_nested_training_and_calibration_entropy_choice():
    protocol = runner._load_frozen_config()
    calibration, _ = runner._validate_calibration_evidence()
    execution = runner._execution_config(protocol, calibration)
...
def test_starting_base_requires_merged_frontier_protocol_ancestor_and_returns_current_head(
    monkeypatch,
):
...
def test_optimizer_identity_audit_rejects_missing_extra_and_duplicate_parameters():
    ...
    reversed_audit = audit_router_optimizer(model, torch.optim.AdamW([second, first], lr=1e-3))
...
def test_fresh_adamw_four_step_and_freeze_audits_are_required_for_gate():
    ...
    aggregate = runner._aggregate_trials(trials, inherited_regressions_status="passed")
...
def test_aggregate_uses_within_seed_pairing_before_medians_and_frontier_membership():
    aggregate = runner._aggregate_trials(_complete_trials(), inherited_regressions_status="passed")
...
def test_gate_boundary_cases_are_classified_without_rule_changes():
    trials = _complete_trials()
    assert (
        runner._aggregate_trials(trials, inherited_regressions_status="missing")["classification"]
        == "PAUSE"
    )
    assert (
        runner._aggregate_trials(trials, inherited_regressions_status="failed")["classification"]
        == "REVISE"
    )
    assert (
        runner._aggregate_trials(
    ...
    assert (
        runner._aggregate_trials(collapsed, inherited_regressions_status="passed")["classification"]
        == "REFINE"
    )
