...
DEFAULT_PROMPTS = ROOT / "configs" / "baseline_evaluation_prompts.json"
...
def load_protocol(config_path: Path = DEFAULT_CONFIG) -> tuple[dict[str, Any], dict[str, Any], str]:
    ...
    frozen_prompt_path = config["fixed_inputs"]["path"]
    prompt_path = (
        ROOT / _FROZEN_PATH_ALIASES.get(frozen_prompt_path, frozen_prompt_path)
    ).resolve()
...
def resolve_modes(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if tuple(ids) != EXPECTED_MODE_IDS:
        raise EvaluationRunnerError(
            f"frozen mode IDs must be exactly {EXPECTED_MODE_IDS}; got {ids}"
        )
...
def frozen_latency_repeats(config: dict[str, Any]) -> int:
    ...
    if repeats != 5:
        raise EvaluationRunnerError(
            f"S09 latency adapter drift: expected five repeats, got {repeats}"
        )
    return repeats
...
def child_command(config_path: Path, mode_id: str, output_path: Path, device: str) -> list[str]:
    return [
        sys.executable,
        str(ROOT / "scripts" / "run_baseline_evaluation.py"),
        "--execute-mode",
        mode_id,
        "--config",
        str(config_path),
...
def plan(config_path: Path, output_dir: Path, device: str) -> dict[str, Any]:
    ...
    commands = [
        child_command(config_path.resolve(), mode["id"], output, device)
        for mode, output in zip(modes, outputs, strict=True)
    ]
    aggregation = [
        sys.executable,
        str(ROOT / "scripts" / "run_baseline_evaluation.py"),
...
def _validate_latency(result: dict[str, Any], repeats: int) -> None:
    ...
    if latency.get("warmup_requests") != 1 or latency.get("repeats_per_request") != repeats:
        raise EvaluationRunnerError(
            "latency warm-up or repeat count does not match the frozen protocol"
        )
    ...
    if set(counts) != set(request_ids) or any(count != repeats for count in counts.values()):
        raise EvaluationRunnerError(
            "latency raw records do not retain exactly five repeats per request"
        )
    for record in raw:
        if not isinstance(record, dict):
    ...
    for request_id in request_ids:
        ...
        for field in ("prefill", "decode", "end_to_end"):
            ...
            if value != expected:
                raise EvaluationRunnerError(
                    f"latency median does not match raw repeats: {request_id}.{field}"
                )
    if (
        latency.get("outlier_removal") is not False
        or latency.get("subtract_transfer_time") is not False
    ):
        raise EvaluationRunnerError("latency filtering or transfer subtraction is not allowed")


def _validate_generation(
    result: dict[str, Any], requests: list[dict[str, Any]], config: dict[str, Any]
) -> None:
    ...
    if not isinstance(records, list) or {
        item.get("request_id") for item in records if isinstance(item, dict)
    ...
    for record in records:
        if not isinstance(record, dict) or record.get("input_ids_sha256") != expected_inputs.get(
            record.get("request_id")
        ):
            raise EvaluationRunnerError("generation record does not use the committed input IDs")
        if (
            not isinstance(record.get("generated_token_ids"), list)
            or len(record["generated_token_ids"]) > expected["max_new_tokens"]
        ):
        ...
        if not isinstance(record.get("finite_value_check"), bool) or not isinstance(
            record.get("normal_termination"), bool
        ):
...
def _validate_routed(result: dict[str, Any], requests: list[dict[str, Any]]) -> None:
    ...
    for record in records:
        ...
        keys = {
            (item.get("layer"), item.get("unit_type"))
        ...
        for field, unit_type in (
            ("attention_fractions", "attention"),
            ("ffn_fractions", "ffn"),
            ("overall_fractions", None),
        ):
            values = record[field]
            selected = (
                route_map
                if unit_type is None
                else [item for item in route_map if item["unit_type"] == unit_type]
            )
            expected_fractions = {
                "4_bit": sum(item["selected_bits"] == 4 for item in selected) / len(selected),
                "8_bit": sum(item["selected_bits"] == 8 for item in selected) / len(selected),
            }
...
def _validate_on_demand(result: dict[str, Any]) -> None:
    ...
    for field in (
        "all_source_qweights_cpu",
        "all_source_luts_cpu",
        "no_complete_packed_gpu_copy",
        "all_repeats_passed",
    ):
        if not isinstance(audit.get(field), bool):
            raise EvaluationRunnerError(f"on-demand hidden-copy audit field is invalid: {field}")
    if (
        audit["any_precision_module_count"] != 0
        or not audit["all_source_qweights_cpu"]
        or not audit["all_source_luts_cpu"]
        or not audit["no_complete_packed_gpu_copy"]
        or not audit["all_repeats_passed"]
    ):
...
def _validate_hardware(result: dict[str, Any], config: dict[str, Any]) -> None:
    ...
    if (
        hardware.get("device_index") != expected_index
        or hardware.get("gpu_model") != expected_model
    ):
...
def _validate_deterministic_evidence(result: dict[str, Any]) -> None:
    ...
    if (
        checks.get("fixed_inputs_identical") is not True
        or checks.get("all_required_outputs_finite") is not True
    ):
        raise EvaluationRunnerError("deterministic evidence is incomplete")
    ...
    if {record.get("request_id") for record in evidence if isinstance(record, dict)} != set(
        request_ids
    ):
    ...
    for record in evidence:
        if not isinstance(record, dict) or record.get("repeat_count") != 5:
            raise EvaluationRunnerError(
                "deterministic repeat evidence must retain five measured repeats"
            )
        if (
            record.get("input_ids_identical") is not True
            or record.get("all_outputs_finite") is not True
            or record.get("generated_outputs_agree") is not True
        ):
        ...
        if (
            not isinstance(record.get("generated_token_ids"), list)
            or len(record["generated_token_ids"]) != 5
        ):
            raise EvaluationRunnerError("deterministic generated-repeat evidence is missing")
        if not isinstance(record.get("route_map_digests"), list) or len(
            record["route_map_digests"]
        ) not in (0, 5):
            raise EvaluationRunnerError("deterministic route-repeat evidence is missing")


def validate_result(
    result: dict[str, Any], config: dict[str, Any], prompts: dict[str, Any], config_hash: str
) -> None:
    ...
    if (
        ...
        or not isinstance(provenance_value.get("git_commit"), str)
        or "worktree_status" not in provenance_value
    ):
    ...
    if (
        not isinstance(result.get("seed"), int)
        or result["seed"] != config["seeds"]["global_reproducibility_seed"]
    ):
        raise EvaluationRunnerError("seed identity is incomplete")
    fixed = result.get("fixed_inputs")
    expected_input_digests = {
        item["id"]: item["input_ids_sha256"] for item in fixed_requests(prompts)
    }
    ...
    if (
        ...
        or identities.get("model_repository") != config["identities"]["model"]["repository"]
        or identities.get("model_revision") != MODEL_REVISION
        or identities.get("tokenizer_revision") != MODEL_REVISION
    ):
        raise EvaluationRunnerError("model or tokenizer revision mismatch")
    if mode["packed_artifact"]:
        if (
            identities.get("packed_checkpoint_sha256")
            != config["identities"]["packed_artifact"]["sha256"]
        ):
    ...
    if (
        mode_id in ROUTED_MODE_IDS
        and identities.get("router_checkpoint_sha256") != config["identities"]["router"]["sha256"]
    ):
        raise EvaluationRunnerError("router checkpoint SHA-256 mismatch")
    if result.get("fixed_inputs", {}).get("request_ids") != [
        item["id"] for item in fixed_requests(prompts)
    ]:
...
def _route_digest(route_map: list[dict[str, Any]]) -> str:
    payload = json.dumps(
        sorted(route_map, key=lambda item: (item["layer"], item["unit_type"])),
        sort_keys=True,
        separators=(",", ":"),
    ).encode()
...
def _route_record(state: Any, request_id: str) -> dict[str, Any]:
    route_map = [
        *[
            {"layer": layer, "unit_type": "attention", "selected_bits": int(bits)}
            for layer, bits in enumerate(state.attention_routes)
        ],
        *[
            {"layer": layer, "unit_type": "ffn", "selected_bits": int(bits)}
    ...
    def fractions(items: list[dict[str, Any]]) -> dict[str, float]:
        count = len(items)
        return {
            "4_bit": sum(item["selected_bits"] == 4 for item in items) / count,
            "8_bit": sum(item["selected_bits"] == 8 for item in items) / count,
        }
    ...
    return {
        ...
        "attention_fractions": fractions(
            [item for item in route_map if item["unit_type"] == "attention"]
        ),
...
def _route_diversity(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    changed_units = (
        sum(len({route[index]["selected_bits"] for route in ordered}) > 1 for index in range(72))
        if ordered
        else 0
    )
    distances = [
...
def _memory_method() -> dict[str, Any]:
    return {
        ...
        "quantities": [
            "allocated_memory",
            "reserved_allocator_memory",
            "physically_resident_packed_weight_bytes",
            "request_owned_on_demand_bytes",
        ],
    }


def _identity_record(
    config: dict[str, Any], manifest: dict[str, Any], mode: dict[str, Any], router_hash: str | None
) -> dict[str, Any]:
    ...
    for module in model.modules():
        ...
        for name in ("qweight", "lut4", "lut8"):
            ...
            if tensor is None or tensor.device.type != "cuda":
                raise EvaluationRunnerError(
                    f"packed residency evidence is not physically measurable: {name}"
                )
...
def _hidden_copy_audit(model: Any, context: Any) -> dict[str, Any]:
    ...
    all_qweights_cpu = bool(sources) and all(
        source.qweight.device.type == "cpu" for source in sources.values()
    )
    all_luts_cpu = bool(sources) and all(
        source.lut4.device.type == "cpu" and source.lut8.device.type == "cpu"
...
def _routed_forward(
    ...
    *,
    ...
    if context is None and any(
        module.__class__.__name__ == "_OnDemandRoutedPackedLinear"
        for module in student.base.modules()
    ):
    ...
    if phase == "prefill":
        kwargs["prompt_attention_mask"] = __import__("torch").ones(
            (1, prompt_length), dtype=__import__("torch").bool, device=device
        )
        ...
        return (
            {
                ...
                "generated_token_ids": sequence[len(request["input_ids"]) :],
                "output_digest": hashlib.sha256(
                    json.dumps(sequence, separators=(",", ":")).encode()
                ).hexdigest(),
                "logits_digest": _tensor_digest(list(output.scores)),
                "finite_value_check": scores_finite
                and bool(torch.isfinite(output.sequences).all().item()),
                "normal_termination": True,
            },
            None,
            None,
        )
    output, state, context = _routed_forward(
        student, request["id"], input_ids, len(request["input_ids"]), device, use_cache=True
    )
    generated = []
    logits = []
    ...
    return (
        {
            ...
            "logits_digest": _tensor_digest(logits),
            "finite_value_check": finite
            and all(bool(torch.isfinite(value).all().item()) for value in logits),
            "normal_termination": True,
        },
        route,
        context,
...
def _perplexity_adapter(model: Any, mode_id: str, device: str) -> Any:
    ...
    class RoutedAdapter:
        def __call__(self, *, input_ids: Any, use_cache: bool = False) -> Any:
            output, state, _ = _routed_forward(
                model,
                f"s09-ppl-{id(input_ids)}",
                input_ids,
...
def _load_mode(
    mode: dict[str, Any], config: dict[str, Any], device: str
    ...
    snapshot = Path(
        os.environ.get(
            "QAQ_MODEL_SNAPSHOT",
            f"~/.cache/huggingface/hub/models--Qwen--Qwen3-4B/snapshots/{MODEL_REVISION}",
        )
    ).expanduser()
    tokenizer = AutoTokenizer.from_pretrained(
        str(snapshot if mode["model_kind"] == "full_precision" else artifact),
    ...
    if mode["id"] in (EXPECTED_MODE_IDS[1], EXPECTED_MODE_IDS[2]):
        ...
        set_static_precision(
            model,
            int(mode["precision"][0])
            if isinstance(mode["precision"], str)
            else int(mode["precision"]),
        )
    ...
    checkpoint = Path(
        os.environ.get("QAQ_S07_ROUTER_CHECKPOINT", "~/.cache/qaq/s07b/final_router.pt")
    ).expanduser()
    metadata = RouterCheckpointMetadata(
        model_repository=config["identities"]["model"]["repository"],
        model_revision=MODEL_REVISION,
        quantized_checkpoint_id=manifest["artifact"]["local_path"],
        quantized_checkpoint_hash=f"sha256:{config['identities']['packed_artifact']['sha256']}",
...
def _expected_physical_bytes(context: Any, route_map: list[dict[str, Any]]) -> dict[str, int]:
    ...
    for module_id, source in context.sources.items():
        ...
        value = int(
            ...
            + getattr(source, f"lut{bits}").numel() * getattr(source, f"lut{bits}").element_size()
        )
        total += value
        if unit == "attention":
            attention += value
...
def _measure_request(
    model: Any, student: Any | None, mode_id: str, request: dict[str, Any], device: str, torch: Any
    ...
    if student is None:
        with torch.inference_mode():
            output = model(
                input_ids=input_ids, attention_mask=torch.ones_like(input_ids), use_cache=True
            )
        torch.cuda.synchronize()
    ...
    if student is not None and context is not None:
        retained_before = {
            ...
            "bytes": context.retained_packed_bytes,
        }
        state.end_request()
        retained_after = {
            "entries": context.retained_entry_count,
    ...
    return (
        ...
        {
            "context": context,
            "actual_transfer_bytes": sum(record.transferred_bytes for record in context.records)
            if context is not None
            else 0,
            "expected_transfer_bytes": _expected_physical_bytes(context, route["route_map"])[
                "total"
            ]
            if context is not None and route is not None
            else 0,
            "actual_attention_bytes": sum(
                record.transferred_bytes
                for record in context.records
                if ".self_attn." in record.module_id
            )
            if context is not None
            else 0,
            "actual_ffn_bytes": sum(
                record.transferred_bytes
                for record in context.records
                if ".mlp." in record.module_id
            )
            if context is not None
            else 0,
            "hidden_copy_audit": _hidden_copy_audit(model, context)
            if mode_id == ON_DEMAND_MODE_ID
            else None,
        },
    )
...
def execute_mode(config_path: Path, mode_id: str, output_path: Path, device: str) -> dict[str, Any]:
    ...
    artifact_path = (
        ROOT
        / config["identities"]["packed_artifact"]["relative_path"]
        / config["identities"]["packed_artifact"]["checkpoint_file"]
    )
    if mode["packed_artifact"]:
    ...
    windows, setup = build_perplexity_windows(
        tokenizer, sample_count=perplexity_args["sample_count"], stride=perplexity_args["stride"]
    )
    perplexity = evaluate_perplexity(_perplexity_adapter(model, mode_id, device), windows, device)
    generation_records = []
    route_records = []
    for request in requests:
        record, route, _ = _generate_record(
            model, mode_id, request, device, torch, student=model if routed else None
        )
    ...
    for request in requests:
        ...
        for repeat in range(repeats):
            measured, route, details = _measure_request(
                model, model if routed else None, mode_id, request, device, torch
            )
            ...
            raw_latency.append(
                {
                    ...
                    **{
                        key: measured[key]
                        for key in ("prefill_seconds", "decode_seconds", "end_to_end_seconds")
                    },
                }
            )
            ...
            if mode_id == ON_DEMAND_MODE_ID and details["context"] is not None:
                ...
                transfer_totals["first_use_bytes"] += sum(
                    record.transferred_bytes for record in records if record.event == "first_use"
                )
                transfer_totals["reuse_bytes"] += sum(
                    record.transferred_bytes for record in records if record.event == "reuse"
                )
                transfer_totals["prefill_bytes"] += total
                transfer_totals["attention_bytes"] += sum(
                    record.transferred_bytes
                    for record in records
                    if ".self_attn." in record.module_id
                )
                transfer_totals["ffn_bytes"] += sum(
                    record.transferred_bytes for record in records if ".mlp." in record.module_id
                )
                transfer_totals["first_use_events"] += sum(
                    record.event == "first_use" for record in records
                )
                transfer_totals["reuse_events"] += sum(
                    record.event == "reuse" for record in records
                )
                transfer_totals["independently_expected_physical_bytes"] += expected["total"]
                hidden_copy_audits.append(details["hidden_copy_audit"])
        input_matches = all(
            item.get("input_ids_sha256") == request["input_ids_sha256"] for item in measured_repeats
        )
        ...
        route_digests = (
            []
            if not routed
            else [runner["route_map_digest"] for runner in routes if runner is not None]
        )
        repeat_evidence.append(
            {
                ...
                "generated_token_ids": generated,
                "generated_outputs_agree": len(
                    {json.dumps(value, separators=(",", ":")) for value in generated}
                )
                == 1,
    ...
    if routed:
        route_payload = {
            "requests": route_records,
            "route_diversity": _route_diversity(route_records),
        }
    ...
    median_seconds = {
        request["id"]: {
            field: median(
                record[f"{field}_seconds"]
                for record in raw_latency
    ...
    physical_cleanup_bytes = max(
        (record["retained_before_cleanup"]["bytes"] for record in memory_records), default=0
    )
    output = {
        ...
        "fixed_inputs": {
            "path": config["fixed_inputs"]["path"],
            "request_ids": [item["id"] for item in requests],
            "input_digests": {item["id"]: item["input_ids_sha256"] for item in requests},
        },
        "perplexity": {
            "setup": {
                **setup,
                ...
                "source_window_length": config["perplexity"]["source_window_length"],
                ...
                "labels": config["perplexity"]["labels"],
                "dataset": config["perplexity"]["dataset"],
                "config": config["perplexity"]["config"],
                "revision": config["perplexity"]["revision"],
                "split": config["perplexity"]["split"],
                "tokenizer_revision": config["perplexity"]["tokenizer_revision"],
            },
            **perplexity,
        },
        ...
        "memory": {
            ...
            "physically_resident_packed_weight_bytes": memory_physical_bytes,
            "request_owned_on_demand_bytes": physical_cleanup_bytes
            if mode_id == ON_DEMAND_MODE_ID
            else 0,
        },
        "latency": {
            ...
            "raw_records": raw_latency,
            "median_seconds": median_seconds,
        },
        "deterministic_checks": {
            "all_required_outputs_finite": all(
                item["finite_value_check"] for item in generation_records
            )
            and all(item["all_outputs_finite"] for item in repeat_evidence),
            "fixed_inputs_identical": all(item["input_ids_identical"] for item in repeat_evidence),
    ...
    if mode_id == ON_DEMAND_MODE_ID:
        hidden_copy_audit = {
            **(hidden_copy_audits[-1] if hidden_copy_audits else {}),
            "all_repeats_passed": bool(hidden_copy_audits)
            and all(
        ...
        output["on_demand"] = {
            **transfer_totals,
            "actual_equals_expected": transfer_totals["total_transfer_bytes"]
            == transfer_totals["independently_expected_physical_bytes"],
            "cleanup_records": cleanup_records,
            "retained_entries_before_cleanup": max(
                (item["retained_entries_before_cleanup"] for item in cleanup_records), default=0
            ),
            "retained_buffers_before_cleanup": max(
                (item["retained_buffers_before_cleanup"] for item in cleanup_records), default=0
            ),
            "retained_entries_after_cleanup": max(
                (item["retained_entries_after_cleanup"] for item in cleanup_records), default=0
            ),
            "retained_buffers_after_cleanup": max(
                (item["retained_buffers_after_cleanup"] for item in cleanup_records), default=0
            ),
            "retained_bytes_after_cleanup": max(
                (item["retained_bytes_after_cleanup"] for item in cleanup_records), default=0
            ),
            "no_complete_packed_parent_on_gpu": hidden_copy_audit.get("no_complete_packed_gpu_copy")
            is True,
...
def _aggregate_pairwise(results: dict[str, dict[str, Any]], config: dict[str, Any]) -> list[str]:
    ...
    if any(
        tuple(record.get(field) for field in hardware_fields) != reference
        for record in hardware_records[1:]
    ):
        errors.append("hardware identities are not comparable across modes")
    if any(
        record.get("gpu_model") != config["hardware"]["required_gpu_model"]
    ...
    if [(item["request_id"], item["route_map"]) for item in resident_routes] != [
        (item["request_id"], item["route_map"]) for item in ondemand_routes
    ]:
        errors.append("resident/on-demand route maps mismatch")
    resident_gen = {
        item["request_id"]: (item["generated_token_ids"], item["logits_digest"])
        for item in resident["generation"]["records"]
    }
    ondemand_gen = {
        item["request_id"]: (item["generated_token_ids"], item["logits_digest"])
        for item in ondemand["generation"]["records"]
    }
    ...
    if (
        payload["total_transfer_bytes"] != payload["independently_expected_physical_bytes"]
        or not payload["actual_equals_expected"]
    ):
        errors.append("on-demand transfer accounting mismatch")
    if (
        payload["retained_entries_after_cleanup"]
        or payload["retained_buffers_after_cleanup"]
        or payload["retained_bytes_after_cleanup"]
        or not payload["no_complete_packed_parent_on_gpu"]
    ):
...
def _persist_aggregation(results_dir: Path, payload: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        (results_dir / "aggregation.json").write_text(
            json.dumps(payload, indent=2, sort_keys=True) + "\n"
        )
    except OSError as exc:
        return {
            "classification": "REVISE",
            "errors": [f"cannot persist aggregation.json: {exc}"],
...
def aggregate(config_path: Path, results_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return _persist_aggregation(
            results_dir,
            {
                "classification": "REVISE",
                "errors": [f"frozen protocol validation failed: {exc}"],
                "results_dir": str(results_dir),
            },
        )
    modes = resolve_modes(config)
    ...
    for mode in modes:
        ...
        try:
            ...
            return _persist_aggregation(
                results_dir,
                {"classification": "REVISE", "errors": [str(exc)], "results_dir": str(results_dir)},
            )
    ...
    return _persist_aggregation(
        results_dir,
        {
            "classification": "REVISE" if errors else "CONTINUE",
            "errors": errors,
